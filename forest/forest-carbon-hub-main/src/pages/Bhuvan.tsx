...
import { Globe2, Search, Layers, AlertCircle, MapPin, Key, Image, Loader2, ExternalLink } from "lucide-react";
...
export default function Bhuvan() {
  ...
  // derive initial region from context or first preset
  ...
  // sync with RegionContext when active region changes
  useEffect(() => {
    if (activeRegion) {
  ...
  // fetch LULC stats whenever location or token changes
  useEffect(() => {
    if (!token) return;
    fetchLULC();
  ...
  const fetchLULC = async () => {
    ...
    try {
      ...
      if (!Array.isArray(data) || data.length === 0) {
        setLulcError("No LULC data found for this area. Try a different region or check your token.");
      } else {
        setLulcData(data);
      }
    } catch (err) {
      ...
      if (msg.includes("Failed to fetch") || msg.includes("NetworkError") || msg.includes("CORS")) {
        setLulcError(
          ...
          "The WMS map above still works. For LULC stats, access the API via a backend proxy or directly from Bhuvan portal."
        );
      } else {
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    const match = forestRegions.find(
      r => r.vanVibhag.toLowerCase().includes(q) || r.name.toLowerCase().includes(q) || r.id === q
    );
    if (match) {
      ...
      setLulcError(`Region "${searchInput}" not found in preset list. Try: Satara, Tadoba, Nagpur, Yavatmal, etc.`);
    }
  };
  ...
  const chartData = aggregateByDesc(lulcData).slice(0, 12); // top 12 land-use types

  // ── Maps3D link — toggled by searched region ──────────────────────────────
  const MAPS3D_LONAVALA =
    "https://maps3d.io/editor/3d?ne=18.80328%2C73.48888&sw=18.68657%2C73.31996&type=poly&p=18.79776%2C73.34091_18.73437%2C73.31996_18.68657%2C73.37661_18.71096%2C73.48888_18.80328%2C73.47995&elevationExaggeration=1.5&elevationZoom=11&elevationVerticalResolution=4&elevationDataset=mapterhorn-global&textureZoom=13&hollowed=false&base=20&hollowedThickness=100&buildingsShow=false&buildingsExtrudedHeightExaggerationRatio=1.5&buildingsColor=ffffff&roadsShow=false&roadsColor=000000&roadsWidthScale=1&roadsExtrudedHeight=3&roadsRenderMode=extruded&waterShow=false&waterColor=3399cc&waterExtrudedHeight=1&waterOpacity=0.7&waterRenderMode=extruded&trackWidth=10&trackExtrudedHeight=3&trackRenderMode=extruded&imagerySelected=satlas-superes-2023&groundMaterialMode=imagery&groundColor=ffffff&projection=EPSG%3A3857";
  const MAPS3D_RATNAGIRI =
    "https://maps3d.io/editor/3d?ne=17.08094%2C73.34087&sw=16.94978%2C73.22206&type=hex&p=17.08094%2C73.28146_17.04814%2C73.34087_16.98256%2C73.34085_16.94978%2C73.28146_16.98256%2C73.22208_17.04814%2C73.22206_17.08094%2C73.28146&elevationExaggeration=1.5&elevationZoom=11&elevationVerticalResolution=4&elevationDataset=mapterhorn-global&textureZoom=13&hollowed=false&base=20&hollowedThickness=100&buildingsShow=false&buildingsExtrudedHeightExaggerationRatio=1.5&buildingsColor=ffffff&roadsShow=false&roadsColor=000000&roadsWidthScale=1&roadsExtrudedHeight=3&roadsRenderMode=extruded&waterShow=true&waterColor=3399cc&waterExtrudedHeight=1&waterOpacity=0.7&waterRenderMode=extruded&trackWidth=10&trackExtrudedHeight=3&trackRenderMode=extruded&imagerySelected=satlas-superes-2023&groundMaterialMode=imagery&groundColor=ffffff&projection=EPSG%3A3857";
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1 text-sm">
            Satellite imagery &amp; LULC stats via ISRO Bhuvan · WMS layer: <code className="text-xs bg-secondary px-1 rounded">lulc:MH_LULC50K_1516</code>
          </p>
        </div>
      ...
      <Card className="border-none shadow-md">
        ...
        <CardContent>
          <div className="relative rounded-xl overflow-hidden border border-border bg-muted" style={{ minHeight: 400 }}>
            {mapLoading && !wmsError && (
            ...
            {wmsError ? (
              <div className="flex flex-col items-center justify-center py-16 gap-3 text-muted-foreground">
                ...
                <p className="text-xs max-w-sm text-center">
                  This may be due to a network restriction or CORS policy on Bhuvan's server.
                  The WMS URL is: <code className="bg-secondary px-1 text-xs break-all rounded">{wmsUrl.substring(0, 80)}...</code>
                </p>
                <Button variant="outline" size="sm" onClick={() => { setWmsError(false); setMapLoading(true); }}>
                  Retry
                </Button>
              </div>
            ) : (
              <img
                ...
                onError={() => { setMapLoading(false); setWmsError(true); }}
              />
            )}
          </div>
          <p className="text-xs text-muted-foreground mt-2">
            Source: ISRO Bhuvan WMS · Layer: <code>lulc:MH_LULC50K_1516</code> · SRS: EPSG:4326 ·{" "}
            <a href="https://bhuvan.nrsc.gov.in" target="_blank" rel="noopener noreferrer" className="underline">bhuvan.nrsc.gov.in</a>
          </p>
        </CardContent>
      </Card>
            ...
            <div className="flex gap-2 max-w-lg">
              <div className="flex-1">
                <Label className="text-xs text-muted-foreground mb-1 block">
                  ...
                  <a href="https://bhuvan-app1.nrsc.gov.in/api/lulc250k/" target="_blank" rel="noopener noreferrer" className="underline text-primary">
              ...
              <Button onClick={handleSaveToken} disabled={!tokenInput.trim()} className="self-end">
                Save &amp; Load
              </Button>
            </div>
            {!token && (
              <div className="mt-3 flex items-start gap-2 text-sm text-amber-700 bg-amber-50 dark:bg-amber-900/20 dark:text-amber-300 rounded-lg p-3">
                ...
                <p>LULC stats require a free API token from ISRO Bhuvan. The WMS map above works without a token.</p>
              </div>
            )}
          </CardContent>
        )}
        ...
        <>
          ...
          <div className="flex items-center gap-3">
            <Button onClick={fetchLULC} disabled={lulcLoading} variant="outline">
              {lulcLoading ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Fetching…</> : <><Layers className="mr-2 h-4 w-4" /> Reload LULC Stats</>}
            </Button>
            <p className="text-xs text-muted-foreground">Polygon: {buildPolygonWKT(selectedLat, selectedLng, 0.5).substring(0, 60)}…</p>
          </div>
            ...
            <Card className="border-none shadow-md">
              ...
              <CardContent>
                <ResponsiveContainer width="100%" height={380}>
                  <BarChart data={chartData} layout="vertical" margin={{ left: 150, right: 30, top: 5, bottom: 5 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="hsl(145,20%,88%)" />
                    <XAxis type="number" tick={{ fontSize: 11 }} label={{ value: "Area (sq.km)", position: "insideBottom", offset: -2 }} />
                    <YAxis dataKey="name" type="category" tick={{ fontSize: 11 }} width={145} />
                    <Tooltip formatter={(v: number) => [`${v.toFixed(2)} sq.km`, "Area"]} />
                    <Legend />
                    <Bar dataKey="area" fill="hsl(152,55%,33%)" name="Area (sq.km)" radius={[0, 4, 4, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </CardContent>
            </Card>
          ...
          {lulcData.length > 0 && (
            <Card className="border-none shadow-md">
              <CardHeader>
                <CardTitle className="text-lg">LULC Raw Data — {selectedName}</CardTitle>
              </CardHeader>
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground mt-2">{lulcData.length} records · Source: ISRO Bhuvan LULC 250K API</p>
              </CardContent>
            </Card>
          )}
        </>
